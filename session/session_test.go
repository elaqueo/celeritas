...
func TestSession_InitSessions(t *testing.T) {
	c := &Session{
		CookieLifetime: "100",
		CookiePersist:  "true",
	...
	for rv.Kind() == reflect.Ptr || rv.Kind() == reflect.Interface {
		fmt.Println("For loop:", rv.Kind(), rv.Type(), rv.Interface())
		sessKind = rv.Kind()
		sessType = rv.Type()
	...
	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("wrong kind returned. Expected", reflect.ValueOf(sm).Kind(), "and got", sessKind)
	}
